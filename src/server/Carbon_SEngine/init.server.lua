...
local r = workspace:FindFirstChild("BulletModel") or Instance.new("Folder")
r.Parent = workspace
r.Name = "BulletModel"
		...
		for _, a in pairs(a) do
			if a then
				for _, d in pairs(t:GetChildren()) do
					if d and d.Name == a and f:WaitForChild("Node_Storage"):FindFirstChild(d.PrimaryPart.Name) then
						for _, _ in pairs(f:GetChildren()) do
							if _ and _:IsA("Model") and _.PrimaryPart and _.PrimaryPart.Name == d.PrimaryPart.Name then
								_:Destroy()
						...
						local _ = d:Clone()
						_.Parent = f
						_:SetPrimaryPartCFrame(f:WaitForChild("Node_Storage"):WaitForChild(_.PrimaryPart.Name).CFrame)
						...
						local a
						for _, _ in pairs(_:GetChildren()) do
							if _ and _:IsA("ModuleScript") and _.Name == "AttchMod" then
								a = require(_)
							end
						end
						...
						if _:FindFirstChild("FirePart") then
							...
							_:WaitForChild("FirePart"):WaitForChild("Fire"):Clone().Parent = f:WaitForChild("Grip")
							f:WaitForChild("Grip"):WaitForChild("Fire").SoundId = "rbxassetid://" .. a[e][1]
						...
						if d.PrimaryPart.Name == "OpticNode" then
							if b then
								local _ = b:FindFirstChild("OverideConfig") or Instance.new("BoolValue")
								_.Parent = b
								_.Name = "OverideConfig"
								_.Value = true
								local _ = b:FindFirstChild("oAimZoom") or Instance.new("NumberValue")
								_.Parent = b
								_.Name = "oAimZoom"
								_.Value = a.AimZoom
								local _ = b:FindFirstChild("oAltAimZoom") or Instance.new("NumberValue")
								_.Parent = b
								_.Name = "oAltAimZoom"
								_.Value = a.AltZoom
								local _ = b:FindFirstChild("oHopUp") or Instance.new("NumberValue")
								_.Parent = b
								_.Name = "oHopUp"
								_.Value = a[e][1]
							end
						...
						elseif d.PrimaryPart.Name == "GripNode" then
							local _ = b:FindFirstChild("OveridePos") or Instance.new("BoolValue")
							_.Parent = b
							_.Name = "OveridePos"
							_.Value = true
							local _ = b:FindFirstChild("ArmPos1") or Instance.new("CFrameValue")
							_.Parent = b
							_.Name = "ArmPos1"
							_.Value = a[e][1]
							local _ = b:FindFirstChild("ArmPos2") or Instance.new("CFrameValue")
							_.Parent = b
							_.Name = "ArmPos2"
							_.Value = a[e][2]
	...
	function(m, c, i, l, d, e, f, b, a, _, g, h)
		...
		if c then
			...
			if o then
				...
				for _, _ in pairs(
					p:WaitForChild(m.UserId):WaitForChild("Attachments"):WaitForChild("Primary_Attachments"):GetChildren(
				...
				for _, _ in pairs(
					p:WaitForChild(m.UserId):WaitForChild("Attachments"):WaitForChild("Secondary_Attachments"):GetChildren(
			...
			m.Character.Torso:WaitForChild("Right Shoulder").Part1 = m.Character["Right Arm"]
			m.Character.Torso:WaitForChild("Left Shoulder").Part1 = m.Character["Left Arm"]
...
v.updateCharEvent.OnServerEvent:connect(
	function(e, c, _, a, d, b)
		if not a or a ~= "Auth" then
			e:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
		end
...
v.halfStepEvent.OnServerEvent:connect(
	function(c, b, a, _)
		if not b or b ~= "Auth" then
			c:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
		end
...
v.damageEvent.OnServerEvent:connect(
	function(d, e, _, b, c)
		if not c[1] or not c[2] or c[2] ~= "Auth" then
			d:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
		end
		...
		if d.Character and d.Character.Humanoid and d.Character.Humanoid.Health > 0 then
			if e.Parent:FindFirstChild("Block") or e.Parent:FindFirstChild("EBlock") then
				...
				if c:FindFirstChild("CE_Medical") and c:FindFirstChild(b) then
					...
					_.Parent = c:WaitForChild("CE_Medical"):WaitForChild("Effects")
				end
				local _ = d.Character:WaitForChild("CarbonValues"):WaitForChild("LastHum").Value
				if e.Health <= 0 and e ~= _ then
					if q and q:FindFirstChild(d.Name) then
						local _ = q[d.Name]
						_.Value = not _.Value
					end
					d.Character:WaitForChild("CarbonValues"):WaitForChild("LastHum").Value = e
					if
					...
					then
						local _ = {
							content = d.Name .. " Has Killed " .. game.Players:GetPlayerFromCharacter(e.Parent).Name
						}
						_ = a:JSONEncode(_)
...
v.whizEvent.OnServerEvent:connect(
	function(i, _, c, b, a, e, g, f, h, d)
		if not h or h ~= "Auth" then
			i:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
		end
...
v.hitEvent.OnServerEvent:connect(
	function(b, f, j, a, _, _, k, o)
		if not _ or _ ~= "Auth" then
			b:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
		end
		if r and o and o.Name ~= "EShield" then
			...
			for _, _ in pairs(game.Players:GetChildren()) do
				if
					_:IsA("Player") and _.Character and _.Character:FindFirstChild("HumanoidRootPart") and
...
v.flybyEvent.OnServerEvent:connect(
	function(a, b, _, _, _, _, _)
		if not _ or _ ~= "Auth" then
			a:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
		end
...
function v.storeDataEvent.OnServerInvoke(s, p, r, l, j, k, i, g, h, n, f, m, o, e, a, _, c, b, d, q)
	if not m or m ~= "Auth" then
		s:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
	end
	if p == "Retrieve" then
		local _
		if
		...
		then
			...
			_.Name = "SightsCycled"
			_.Value = false
			local d = f:FindFirstChild("OverideConfig") or Instance.new("BoolValue", f)
			d.Name = "OverideConfig"
			local c = f:FindFirstChild("oAimZoom") or Instance.new("NumberValue", f)
			c.Name = "oAimZoom"
			local b = f:FindFirstChild("oAltAimZoom") or Instance.new("NumberValue", f)
			b.Name = "oAltAimZoom"
			local e = f:FindFirstChild("oHopUp") or Instance.new("NumberValue", f)
			e.Name = "oHopUp"
			local _ = f:FindFirstChild("OveridePos") or Instance.new("BoolValue", f)
			_.Name = "OveridePos"
			local a = f:FindFirstChild("ArmPos1") or Instance.new("CFrameValue", f)
			a.Name = "ArmPos1"
			local f = f:FindFirstChild("ArmPos2") or Instance.new("CFrameValue", f)
			f.Name = "ArmPos2"
...
v.medEvent.OnServerEvent:connect(
	function(b, a, _, _, _, _, _, _, _, _, _, _, _)
		...
		if a then
			b.Character:WaitForChild("Humanoid").Health = b.Character:WaitForChild("Humanoid").MaxHealth
		end
	end
)
v.explosiveEvent.OnServerEvent:connect(
	function(h, f, g, b, d, c, _, _, a, _, _, _, _, e, _)
		if not e or e ~= "Auth" then
			h:Kick("Exploiting is a bannable offense. This action log has been submitted to ROBLOX.")
		end
		...
		if e.DestroyJointRadiusPercent <= 0 then
			for _, _ in pairs(game.Players:GetChildren()) do
				if _ and _:IsA("Player") then
					if
						_.Character and _.Character:FindFirstChild("HumanoidRootPart") and
								...
								a.Died:Connect(
									function()
										if game.Players:GetPlayerFromCharacter(a.Parent) then
											...
											game.Debris:AddItem(_, 0.1)
											local _ =
												h.Character:WaitForChild("CarbonValues"):WaitForChild("LastHum").Value
			...
			for _, _ in pairs(workspace:GetDescendants()) do
				if _ and _:IsA("BasePart") then
					if _ and (_.Position - g).magnitude <= d then
						...
						if _ and (_:IsA("IntValue") or _:IsA("NumberValue")) then
							_.Value = _.Value - 500
							break
						end
					end
...
v.attachmentEvent.OnServerEvent:Connect(
	function(f, b, d)
		if
			...
			f.Character:FindFirstChild("@" .. b):FindFirstChild("Node_Storage")
		then
